...
function assignAxisFacePixels(target, faceKey, bounds, includeOpposite = true) {
				...
				if ((overlayMask[adjFaceKey]?.[row]?.[adjCol])?.active) {
					...
					if (!emittedCaps.has(vcCapKey)) {
						...
						if (row === 0) {
							...
							if (topInfo?.active) assignAxisFacePixels(capFaceBounds, "top", topInfo, false);
						} else if (row === 7) {
							const bpx = def.bottomPixel;
							const bottomInfo = bpx ? overlayMask["bottom"]?.[bpx.row]?.[bpx.col] : null;
							if (bottomInfo?.active) assignAxisFacePixels(capFaceBounds, "bottom", bottomInfo, false);
						}
