...
function assignAxisFacePixels(target, faceKey, bounds) {
...
function buildVoxelizedOverlayWithRelief(skinImage, heightmap) {
			...
			if (vcr) {
				...
				if ((overlayMask[adjFaceKey]?.[row]?.[adjCol])?.active) {
					...
					if (!emittedCaps.has(vcCapKey)) {
						...
						const capFaceBounds = {};
						assignAxisFacePixels(capFaceBounds, def.faceA, piA);
						assignAxisFacePixels(capFaceBounds, def.faceB, piB);
						if (row === 0) {
							const tpx = def.topPixel;
							const topInfo = tpx ? overlayMask["top"]?.[tpx.row]?.[tpx.col] : null;
							if (topInfo?.active) assignAxisFacePixels(capFaceBounds, "top", topInfo);
						} else if (row === 7) {
							const bpx = def.bottomPixel;
							const bottomInfo = bpx ? overlayMask["bottom"]?.[bpx.row]?.[bpx.col] : null;
							if (bottomInfo?.active) assignAxisFacePixels(capFaceBounds, "bottom", bottomInfo);
						}
						const capGeom = buildBoxGeometryWithFacePixels(THICKNESS, capH, THICKNESS, cpi, capFaceBounds);
				...
				if (heRole !== null) {
					...
					if (!emittedCaps.has(heCapKey)) {
						...
						if (piA?.active && piB?.active) {
							...
							const capFaceBounds = {};
							assignAxisFacePixels(capFaceBounds, hDef.faceA, piA);
							assignAxisFacePixels(capFaceBounds, hDef.faceB, piB);
							const capGeom = buildBoxGeometryWithFacePixels(cW, hDef.capH, cD, cpi, capFaceBounds);
	...
	for (const tc of [
		...
		if (i1?.active && i2?.active && i3?.active) {
			...
			const tcFaceBounds = {};
			assignAxisFacePixels(tcFaceBounds, tc.f1, i1);
			assignAxisFacePixels(tcFaceBounds, tc.f2, i2);
			assignAxisFacePixels(tcFaceBounds, tc.f3, i3);
			const tcFaceGeom = buildBoxGeometryWithFacePixels(THICKNESS, THICKNESS, THICKNESS, i1, tcFaceBounds);
